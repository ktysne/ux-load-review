...
UI の記憶、判断、操作の負担に関する UX 規則をチェックし適用するエージェントスキル `ux-load-review` を管理する。
スキルの形式は Claude Code と Codex で共通であり、どちらか一方に依存する記述を入れない。
スキル本体は `skills/ux-load-review/` にあり、利用者はこのディレクトリを使うエージェントのスキルディレクトリへコピーして使う。

同じ立ち位置の姉妹スキルとして、UI の状態（待つ、空、失敗、権限不足、削除、完了、入力不備）を扱う `ux-state-review` がある。
状態の設計に関する話題はそちらに譲り、このスキルでは扱わない。
...
- 「対象外」と「別の表現で満たす条件」を混ぜない。前者は問題が起きないか困らない場合、後者は別の手段で目的を達成できる場合である。
- 規則の一覧表は `skills/ux-load-review/SKILL.md` と `README.md` の両方にある。片方を変えたらもう一方も揃える。
- 規則を変更したら `skills/ux-load-review/evals/evals.json` に、その変更を検証できるテストプロンプトを追加する。
